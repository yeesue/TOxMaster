...
Can_Fun_Thread_TS::Can_Fun_Thread_TS(QObject *parent)
{

}

Can_Fun_Thread_TS::~Can_Fun_Thread_TS()
{
    stopCan();
}

void Can_Fun_Thread_TS::setFunInfo(FunStruct *value, int index)
{
    fun = value;
    funIndex = index;
}

void Can_Fun_Thread_TS::run()
{
    init();
...
void Can_Fun_Thread_TS::Slt_CanStateChanged(quint32 canIndex, Can_Thread_TS::CAN_STATE state)
{
    if(state != curState)
    {
        curState = state;
        ...
        int funState = 0;
        if(curState == Can_Thread_TS::CONN_ERR || curState == Can_Thread_TS::START_ERR || curState == Can_Thread_TS::R_W_ERR)
        {
            funState = -1;
        }
        else if(curState == Can_Thread_TS::CONN_OK)
        {
            funState = 1;
        }
        else if(curState == Can_Thread_TS::START_OK)
        {
            funState = 2;
        }
        else if(curState == Can_Thread_TS::R_W_ING)
        {
            funState = 3;
        }
        else
...
void Can_Fun_Thread_TS::Slt_ReadFrameRawDataReceived(quint32 id, quint8 *data, quint8 len)
{
    ...
    QString keyName = "RP_TSCAN" + QString::number(canIndex) + "_" + QString::number(id);
...
void Can_Fun_Thread_TS::Slt_FrameSigValueUpdated()
{
...
void Can_Fun_Thread_TS::Slt_WriteFrameSigValueUpdated()
{
...
void Can_Fun_Thread_TS::init()
{
    if(tscanIns == NULL)
    {
        tscanIns = new Can_Thread_TS();
    }

    connect(tscanIns, SIGNAL(stateChanged(quint32, Can_Thread_TS::CAN_STATE)), this, SLOT(Slt_CanStateChanged(quint32, Can_Thread_TS::CAN_STATE)));
    //connect(tscanIns, SIGNAL(readFrameRawDataReceived(quint32, quint8*, quint8)), this, SLOT(Slt_ReadFrameRawDataReceived(quint32, quint8*, quint8)));
    //connect(tscanIns, SIGNAL(readFrameRawDataReceived(quint32, quint8*, quint8)), this, SLOT(Slt_ReadFrameSmUpdated(quint32,quint8*,quint8)));
    connect(tscanIns, SIGNAL(canDataForRecord(quint8*,quint32,QString)), this, SIGNAL(canDataForRecord(quint8*,quint32,QString)));
...
bool Can_Fun_Thread_TS::startCan()
{

    if(!tscanIns) return false;

    tscanIns->start();

    //wait for can state changed
    waitForCanThreadStateReady(tscanIns, 3000);

    canStartFinished = true;

    this->readFrameSmHash = tscanIns->getReadFrameSmHash();
    this->writeFrameSmHash = tscanIns->getWriteFrameSmHash();
...
void Can_Fun_Thread_TS::stopCan()
{
    ...
    if(tscanIns)
    {
        disconnect(tscanIns, SIGNAL(readFrameRawDataReceived(quint32, quint8*, quint8)), this, SLOT(Slt_ReadFrameRawDataReceived(quint32, quint8*, quint8)));

        tscanIns->setIsStop(true);
        tscanIns->quit();
        tscanIns->wait();
...
void Can_Fun_Thread_TS::waitForCanThreadStateReady(Can_Thread_TS *thread, quint32 waitTime_ms)
{
    if(!thread)
        return;
    int count = waitTime_ms / 500;
    int index = 0;
    while(!(thread->getCurState() == Can_Thread_TS::R_W_ING) && index <= count)
    {
        wait_ms(500);
...
void Can_Fun_Thread_TS::wait_ms(int msec)
{
...
BO_ *Can_Fun_Thread_TS::getBoById(quint32 id)
{
    if(m_cluster == NULL)
        return NULL;
...
SG_ *Can_Fun_Thread_TS::getSgByName(BO_ *bo, QString sgName)
{
    if(!bo)
        return NULL;
...
bool Can_Fun_Thread_TS::loadXml()
{
    dbc_frames_r.clear();
    dbc_frames_w.clear();
    ...
    canIoMode = (root.attribute("can_io_mode").toInt());
    canfdStandard = (root.attribute("canfd_standard").toInt());
...
void Can_Fun_Thread_TS::setCanCommInfo()
{
    if(!tscanIns)
        return;

    tscanIns->setCanIndex((APP_CHANNEL)canIndex);
    tscanIns->setCanfd_abitBaudrate(canBaudrate);
    tscanIns->setCanfd_dbitBaudrate(canfdBaudrate);
    tscanIns->setResistanceEnable(terminationEnable);
    tscanIns->setCanType(canIoMode > 0 ? 1 : 0);
    tscanIns->setCanfd_standard(canfdStandard);
    tscanIns->setIsCustomBaud(enableCustomBD);

    tscanIns->setReadFrameList(this->dbc_frames_r);
    tscanIns->setWriteFrameList(this->dbc_frames_w);

}


void Can_Fun_Thread_TS::loadDbc(QString dbcFile)
{
    if(dbcFile.isEmpty())
        return;
...
QList<PARAM *> Can_Fun_Thread_TS::fromDbcFrameSigsToPams(BO_ *frame, QString key)
{
    QList<PARAM*> pamList;
    if(!frame)
        return pamList;
...
void Can_Fun_Thread_TS::packFramesIntoPamsHash()
{
    for(int i = 0; i < dbc_frames_r.count(); i++)
    {
        BO_ *frame = dbc_frames_r.at(i);
...
QHash<QString, QList<PARAM *> > Can_Fun_Thread_TS::getWritePamListHash() const
{
    return writePamListHash;
}

bool Can_Fun_Thread_TS::getCanStartFinished() const
{
    return canStartFinished;
}

QHash<QString, QList<PARAM *> > Can_Fun_Thread_TS::getReadPamListHash() const
{
