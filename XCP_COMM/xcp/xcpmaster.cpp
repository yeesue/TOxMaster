    ...
    QObject(parent), xcpName(name), xcpDeviceType(canType)
{
    ...
    else if(xcpDeviceType == 3) // Unified driver
    {
        // Create unified thread with default driver type (NI-XNET)
        xcpUnifiedThread = new xcp::XCP_Thread_Unified(can::CanDriverFactory::DriverType::NI_XNET, this);
        // 暂时注释掉信号连接，因为参数类型不匹配
        // connect(xcpUnifiedThread->readThread(), &xcp::XCP_R_Thread_Base::RESDataReady, this, &XCPMaster::getCTORESData);
    }
...
XCPMaster::~XCPMaster()
{
    ...
    if(xcpZlgCanThread)
    {
        delete xcpZlgCanThread;
    }
    if(xcpUnifiedThread)
    {
        delete xcpUnifiedThread;
    }
...
bool XCPMaster::XCPInit()
{
    ...
    */
    ...
    else if(xcpDeviceType == 3)
    {
        qInfo()<<"ѡ��统一驱动接口";
        return XCPInit_Unified();
    }
...
bool XCPMaster::XCPInit_Unified()
{
    if (!xcpUnifiedThread)
        return false;

    // Set configuration
    xcpUnifiedThread->setCanInterface(xcpcan_Interface);
    xcpUnifiedThread->setBaudRate(xcpBaudrate);
    xcpUnifiedThread->setFdBaudRate(xcpFdBaudrate);
    xcpUnifiedThread->setIsCanFd(isCanFd);
    xcpUnifiedThread->setMaxDLC(maxDLC);
    xcpUnifiedThread->setResistanceEnable(resistanceEnable);
    xcpUnifiedThread->setIdMasterToSlave(ID_Master_to_Slave);
    xcpUnifiedThread->setIdSlaveToMaster(ID_Slave_to_Master);
    xcpUnifiedThread->setEventIdList(idList);

    // Initialize
    bool success = xcpUnifiedThread->XCP_CAN_Init();
    xcpCanInitFlag = success;
    return success;
}

void XCPMaster::XCPStop_Unified()
{
    if (xcpUnifiedThread)
    {
        xcpUnifiedThread->XCP_CAN_Stop();
...
bool XCPMaster::XCPInit_NI()
{
    ...
    qDebug()<<"XCP��ʼ�� : ��ʼ��CAN�˿�״̬:"<<xcpCanInitFlag;
...
{
    qDebug()<<"XCP��ʼ��:TS_CAN CanIndex:"<<this->xcpcan_Interface;
...
{
    qDebug()<<"XCP��ʼ��:ZLG_CAN DeviceType:"<<zlgDeviceType
...
bool XCPMaster::XCP_Poll_Meas(A2L_VarMeas *measVar)
{
    ...
    memcpy(data.data(), (char*)data_UPLOAD, size);

    if(ByteOrder_XCPAddr == 1)
        reverse(data.data(), size);

    return copyPollRawDataToSM(data.data(), size, measVar);
...
bool XCPMaster::copyPollRawDataToSM(char *data, int size, A2L_VarMeas *measVar)
{
    ...
    int memorySize = 8 + measVar->StartByte + size;
    void* memory = MemoryManager::instance()->createMemory(measVar->smKey, memorySize);
    
    if(!memory)
    {
        qDebug()<<"Unable to create memory.";
        return false;
    }
    
    // 复制数据到内存
    memcpy((quint8*)memory, CTO_RESTime, 8);
    memcpy((char*)memory + 8 + measVar->StartByte, data, size);
...
void XCPMaster::arrangeMeasVars()
{
    ...
    for (int i = 0; i < daqMeasVarList.count(); i++)
    {
        ...
        sizeDataBuffer_DAQ += var->DataSizeAG;                  // ????DAQ ???? ???��
    ...
    //�Բ������ú�DAQ����EVENT�������������Ϊ�п��ܳ������ڴ��EVENT��ŷ���С
    //��������²���������Ļ��ᵼ��EVENT�������ã���FD 06(DAQ overload)
    ...
    for (int i = 0; i < keys.count(); i++)
    {
        ...
        // 使用MemoryManager替代QSharedMemory
        QString key = QString("DAQ_%1_%2").arg(xcpName).arg(curDAQList);
        MemoryManager::instance()->createMemory(key, dataSize + 8);
...
{    
    ...
    if(isCanFd && (maxDLC == 64))
    {
        ...
        // 使用MemoryManager替代QSharedMemory
        QString key = "RP_DAQ_" + xcpName + "_" + QString::number(DAQList);
        ...
        quint8 *to = static_cast<quint8*>(memory);
        quint8 *from = DAQ_ODTPacket+1;
        // 直接复制数据，不需要锁定，因为MemoryManager内部已经处理了线程安全
        memcpy(to, time, 8);
        ...
        if (attr.biggestPID)
        {
            ...
            emit ODTDataForRecord(buf_rcd, smSize, key);
        }
    }
    else
    {
...
void XCPMaster::getDAQODTData_TS(quint8 *data, quint32 numData)
{
    ...
    // 使用MemoryManager替代QSharedMemory
    QString key = QString("DAQ_%1_%2").arg(xcpName).arg(DAQList);
    quint32 smSize = daqListBlockSizeHash.value(DAQList);
...
void XCPMaster::ODTDataUpdatedSlot(quint16 DAQList)
{
    ...
    for(int i = 0; i < vars.count(); i++)
    {
        ...
        quint8 *from = static_cast<quint8*>(mem);
...
void XCPMaster::xcpdbcUpdate()
{
    ...
    if (dbcFile.exists())
    {
        ...
        QByteArray clusterName(sizeOfClusterName, Qt::Uninitialized);
        ...
        if (status_dbc == nxSuccess)
        {
            ...
            sel_cluster = QString(QLatin1String(clusterName.constData()));
        ...
        {
            qDebug()<<XCPErrorRegistry::instance().formatError(ERR_INIT_FAILED, "Cluster set can io mode");
        }
        ...
        {
            qDebug()<<XCPErrorRegistry::instance().formatError(ERR_INIT_FAILED, "Cluster set baudrate");
        }
        ...
        if(isCanFd)
        {
            ...
            {
                qDebug()<<XCPErrorRegistry::instance().formatError(ERR_INIT_FAILED, "Cluster set fd baudrate");
            }
            ...
            {
                qDebug()<<XCPErrorRegistry::instance().formatError(ERR_INIT_FAILED, "Cluster set fd ISO mode");
            }
        }
        ...
        for (int i = 0; i < numOfFrameRef; i++)
        {
            ...
            frameName = QByteArray(sizeOfFrameName, Qt::Uninitialized);
...
bool XCPMaster::CTO_CMD_RES_Comm()
{
    if (!xcpCanInitFlag)
    {
        throw XCPException(ERR_NOT_CONNECTED, "CAN device not initialized");
    }
    ...
    if (xcpDeviceType == 0) // NI XNET
    {
        ...
        while ((!xcpCanThread->thread_CMD->writeOnceSucceed()) || (!CTORESDataUpdated))
        {
            ...
            {
                throw XCPException(ERR_TIMEOUT, "CTO timeout > 1000ms (NI XNET)");
            }
        }
        xcpCanThread->thread_RES->setResDataReady(false);

    }
    else if (xcpDeviceType == 1) // TSCANͬ��
    {
        if(!isCanFd)
        {
        ...
        while ((!xcpTsCanThread->thread_CMD->writeOnceSucceed()) || (!CTORESDataUpdated))
        {
            ...
            if (time.elapsed() > 1000)
            {
                throw XCPException(ERR_TIMEOUT, "CTO timeout > 1000ms (TSCAN)");
            }
        }
    ...
    else if (xcpDeviceType == 2) // ZLG_CAN
    {
        ...
        while ((!xcpZlgCanThread->thread_CMD->writeOnceSucceed()) || (!CTORESDataUpdated))
        {
            ...
            if (time.elapsed() > 1000)
            {
                throw XCPException(ERR_TIMEOUT, "CTO timeout > 1000ms (ZLG_CAN)");
            }
        }
    ...
    {
        throw XCPException(ERR_INIT_FAILED, "Invalid CAN device type");
    }
...
bool XCPMaster::CTO_CONNECT()
{
    ...
    try
    {
        if (!CTO_CMD_RES_Comm())
        {
            qDebug()<<XCPErrorRegistry::instance().formatError(ERR_COMMUNICATION, "CONNECT command");
            return false;
        }
    }
...
bool XCPMaster::CTO_DISCONNECT()
{
    ...
    try
    {
        if (!CTO_CMD_RES_Comm())
        {
            qDebug()<<XCPErrorRegistry::instance().formatError(ERR_COMMUNICATION, "DISCONNECT command");
            return false;
        }
    }
    catch (const XCPException& e)
...
bool XCPMaster::CTO_GET_STATUS()
{
    ...
    try
    {
        if (!CTO_CMD_RES_Comm())
        {
            qDebug()<<XCPErrorRegistry::instance().formatError(ERR_COMMUNICATION, "GET_STATUS command");
            return false;
        }
    }
    catch (const XCPException& e)
...
qint16 XCPMaster::XCP_Seed2Key(QString dllPath, quint8 privilege, quint8 lenOfSeed, quint8 *seed, quint8 *lenOfKey, quint8 *key)
{
    ...
    if(lib.load())
    {
        ...
        {
            qDebug()<<XCPErrorRegistry::instance().formatError(ERR_ACCESS_DENIED, "resolve XCP SeedKey");
            qDebug()<<lib.errorString();
        }
    }
    else
    {
        qDebug()<<XCPErrorRegistry::instance().formatError(ERR_ACCESS_DENIED, "load XCP SeedKey") + ": " + lib.errorString();
    }
}
...
bool XCPMaster::CTO_GET_ID()
{
    ...
    CTO_CMDPacket[0] = 0xFA;  // GET_ID ��zlg canframe��Ĺؼ����ظ�,������ԭʼ��ֵ
...
// 注释掉QSharedMemory相关方法的实现，使用MemoryManager替代
/*
QHash<quint16, QSharedMemory *> XCPMaster::getDaqListSMHash() const
{
    // 使用MemoryManager替代QSharedMemory，返回空的QHash
    return QHash<quint16, QSharedMemory *>();
}
*/
