#ifndef CAN_THREAD_ZLG_H
#define CAN_THREAD_ZLG_H


//#define Q_OS_WIN
//#define Q_OS_LINUX

#include <QThread>
#include <QDebug>
#include <QSharedMemory>
#include <QTime>
#include <QTimer>
#include "dbcparser.h"
#include "common.h"

#ifdef Q_OS_WIN
#include "zlgcan.h"
#include "zlgcan_driver.h"
#endif

#ifdef Q_OS_LINUX
#include "zcan.h"
#endif

/*
struct CRC_INFO{
    int crcIndex;
    int startIndex;
    int endIndex;
};
*/

/////////////////////////////////////////////////////////
class Can_R_Thread_ZLG : public QThread
{
    Q_OBJECT
public:
    Can_R_Thread_ZLG(QObject *parent = 0);
    ~Can_R_Thread_ZLG();

#ifdef Q_OS_WIN
    void setChannelHandle(const CHANNEL_HANDLE &value);
#endif

    void setIsStop(bool value);

    void setCanType(const quint32 &value);

    void setReadFrameList(const QList<BO_ *> &value);

    void setModName(const QString &value);

    void setSmHash(const QHash<quint32, QSharedMemory *> &value);

    void setMdfRecordOn(bool value);

    void setDeviceType(const quint32 &value);

    void setDeviceIndex(const quint32 &value);

    void setCanIndex(const quint32 &value);


protected:
    void run();

signals:
    void canDataForRecord(quint8*,quint32,QString);

private:
    void rcvCanDataHandle(quint32 num);
    void rcvCanFdDataHandle(quint32 num);

private:
#ifdef Q_OS_WIN
    CHANNEL_HANDLE channelHandle;
#endif
    quint32 deviceType = 0;
    quint32 deviceIndex = 0;
    quint32 canIndex = 0;

    bool isStop = false;
    bool mdfRecordOn = false;

    quint32 canType = 0;
#ifdef Q_OS_WIN
    ZCAN_Receive_Data *pCanDataRec = NULL;
    ZCAN_ReceiveFD_Data *pCanFDDataRec = NULL;
#endif
#ifdef Q_OS_LINUX
    ZCAN_20_MSG *pCanDataRec_ = NULL;
    ZCAN_FD_MSG *pCanFDDataRec_ = NULL;
#endif
    QList<BO_*> readFrameList;

    QString modName = "";
    QHash<quint32, QSharedMemory*> smHash;

};

/////////////////////////////////////////////////////////
class Can_W_Thread_ZLG : public QThread
{
    Q_OBJECT
public:
    Can_W_Thread_ZLG(QObject *parent = 0);
#ifdef Q_OS_WIN
    void setChannelHandle(const CHANNEL_HANDLE &value);
#endif
    void setIsStop(bool value);

    void setCanType(const quint32 &value);

    void setWriteFrameList(const QList<BO_ *> &value);

    void setModName(const QString &value);

    void setSmHash(const QHash<quint32, QSharedMemory *> &value);

    void setWriteFrame(BO_ *value);

    void setSm(QSharedMemory *value);

    void setCanIndex(const quint32 &value);

    void setDeviceType(const quint32 &value);

    void setDeviceIndex(const quint32 &value);

public slots:

protected:
    void run();

signals:

private:
    void init();
    bool packTransmitData();
    void sigRC(SG_ *sg, int min, int max, int step);
    void dataCRC(char *data, int startIndex, int endIndex, int crcIndex);
    void tableXorCRC(char *data, int startIndex, int endIndex, int crcIndex);

    bool copySndDataFromSmToBuf();
    void updateSndTimeStamp();

    bool packTransmitData_v2();

private:
#ifdef Q_OS_WIN
    CHANNEL_HANDLE channelHandle;
#endif
    quint32 deviceType = 0;
    quint32 deviceIndex = 0;
    quint32 canIndex = 0;

    QTime time;
    //quint64 sndAbsTime = 0;
    qreal sndAbsTime = 0;     // send time(s)

    QString modName = "";

    bool isStop = false;
    bool remoteCtrl = false;

    quint32 canType = 0;

    QList<BO_*> writeFrameList;
    QHash<quint32, QSharedMemory*> smHash;

    BO_ *writeFrame = NULL;
    QSharedMemory *sm = NULL;
    quint16 transRate = 100;

#ifdef Q_OS_WIN
    ZCAN_Transmit_Data pCanDataSnd;
    ZCAN_TransmitFD_Data pCanFDDataSnd;

    ZCANDataObj *pTransmit = NULL;
#endif
#ifdef Q_OS_LINUX
    ZCAN_20_MSG pCanDataSnd_;
    ZCAN_FD_MSG pCanFDDataSnd_;
#endif

    QList<CRC_INFO> crcInfoList;

    //for table xor crc method
    unsigned char crc_Table[256] =
      { 
      };

};

/////////////////////////////////////////////////////////
class Can_Thread_ZLG : public QThread
{
    Q_OBJECT
public:
    Can_Thread_ZLG(QObject *parent = 0);
    ~Can_Thread_ZLG();

    enum CAN_STATE {
        UNOPEN,

        OPEN_OK,
        OPEN_ERR,

        SETVALUE_OK,
        SETVALUE_ERR,

        INIT_OK,
        INIT_ERR,

        SETRES_OK,
        SETRES_ERR,

        START_OK,
        START_ERR,

        R_W_ING,
        R_W_ERR,

        CLOSE_OK,
        CLOSE_ERR,

        ERR,
        STOP,

        END
    };
    Q_ENUM(CAN_STATE)

    void setIsStop(bool value);

    void setCanStop(bool value);

    void setDeviceType(const quint32 &value);

    void setDeviceIndex(const quint32 &value);

    void setCanIndex(const quint32 &value);

    void setCanType(const quint32 &value);

    void setCanfd_standard(const quint32 &value);

    void setCanfd_abitBaudrate(const quint32 &value);

    void setCanfd_dbitBaudrate(const quint32 &value);
#ifdef Q_OS_WIN
    void setCanPortInitConfig(const ZCAN_CHANNEL_INIT_CONFIG &value);
#endif
    void setResistanceEnable(const quint32 &value);

    void setIsCustomBaud(bool value);

    void setReadFrameList(const QList<BO_ *> &value);

    void setWriteFrameList(const QList<BO_ *> &value);

    void setModName(const QString &value);

    void setReadSmHash(const QHash<quint32, QSharedMemory *> &value);

    void setWriteSmHash(const QHash<quint32, QSharedMemory *> &value);

    void setMdfRcdOn(bool flag);

    Can_Thread_ZLG::CAN_STATE getCurState() const;
#ifdef Q_OS_WIN
    DEVICE_HANDLE getDeviceHandle() const;
    void setDeviceHandle(const DEVICE_HANDLE &value);
#endif
    void setStopWithoutClose(bool value);


    QHash<QString, QSharedMemory *> getReadFrameSmHash() const;

    QHash<QString, QSharedMemory *> getWriteFrameSmHash() const;

protected:
    void run();

signals:
    void stateChanged(quint32, quint32, Can_Thread_ZLG::CAN_STATE);

    void canDataForRecord(quint8*,quint32,QString);

private slots:

private:
    void initConnection();

    void setFiltersForFrames(QList<BO_*> frames);

#ifdef Q_OS_WIN
    void readChannelErrInfo(CHANNEL_HANDLE channelHandle);
#endif
#ifdef Q_OS_LINUX
    void setBaudrateAttr(ZCAN_INIT *init, quint32 abit, float sp_abit, quint32 dbit, float sp_dbit);
#endif

private:
    CAN_STATE curState = UNOPEN;
    CAN_STATE lastState = UNOPEN;
    bool isStop = false;
    bool canStop = false;      // false = can run, true = can stop
    bool stopWithoutClose = false;

    //CanDeviceConfig canConfig;

    QString modName = "";

#ifdef Q_OS_WIN
    DEVICE_HANDLE deviceHandle = INVALID_DEVICE_HANDLE;
    CHANNEL_HANDLE channelHandle = INVALID_CHANNEL_HANDLE;
    ZCAN_CHANNEL_INIT_CONFIG canPortInitConfig;       //config of can
    ZCAN_DEVICE_INFO deviceInfo;
    ZCAN_Receive_Data *pCanDataRec = NULL;
    ZCAN_ReceiveFD_Data *pCanFDDataRec = NULL;
#endif

#ifdef Q_OS_LINUX

#endif

    quint32 deviceType = 0;       // device type like: USBCANFD-200U ...
    quint32 deviceIndex = 0;      // device index: the index of same devices
    quint32 canIndex = 0;         // can port index: the index of can port in a device
    quint32 canType = 0;          // 0=can, 1=canfd

    bool isCANFD = false;

    int canfd_standard = 0;

    bool isCustomBaud = false;

    quint32 canfd_abitBaudrate = 500000;

    quint32 canfd_dbitBaudrate = 2000000;

    int resistanceEnable = 0;

    Can_R_Thread_ZLG *canRcvThread = NULL;
    Can_W_Thread_ZLG *canSndThread = NULL;
    QList<Can_W_Thread_ZLG*> canSndThreadList;

    QList<BO_*> readFrameList;
    QList<BO_*> writeFrameList;

    QHash<quint32, QSharedMemory*> readSmHash;
    QHash<quint32, QSharedMemory*> writeSmHash;

    QHash<QString, QSharedMemory*> readFrameSmHash;
    QHash<QString, QSharedMemory*> writeFrameSmHash;


    // 计划将原来每个报文一个线程的方式改为每种定时周期一个线程，减少线程数量
    // 比如10ms, 20ms, 50ms, 100ms, 200ms, 500ms..最多有这么多的线程
    //经过验证，此种方式定时不够精准
    //QList<QThread*> timerThreadList;
    //QHash<int, QThread*> timerThreadHash;
    //QHash<int, QTimer*> timerHash;

};

#endif // CAN_THREAD_ZLG_H
