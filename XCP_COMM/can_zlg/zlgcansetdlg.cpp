...
#include "ui_zlgcansetdlg.h"
#include <QDir>
#include <QDesktopWidget>
#include "dsbdelegate.h"

ZlgCanSetDlg::ZlgCanSetDlg(QWidget *parent, QString work_name, QString fun_name, QString intf_name, QString set_file_path) :
    QDialog(parent),
    workName(work_name), funName(fun_name),
    intfName(intf_name), canSetFilePath(set_file_path),
    ui(new Ui::ZlgCanSetDlg)
{
    ui->setupUi(this);
...
ZlgCanSetDlg::~ZlgCanSetDlg()
{
    delete ui;
}

void ZlgCanSetDlg::setDatabaseHeader(QString filePath)
{
...
void ZlgCanSetDlg::showClusterInTreeView(CLUSTER_ *cluster)
{
    if(cluster == NULL)
        return;
...
void ZlgCanSetDlg::initDeviceTypeNumHash()
{
#ifdef Q_OS_WIN
...
void ZlgCanSetDlg::setHeader(QString filePath)
{
    QString name;
    QStringList fileList = filePath.split("/", QString::SkipEmptyParts);
    name = fileList.at(fileList.count()-1);
...
void ZlgCanSetDlg::loadDatabase(QString dbFilePath)
{
...
void ZlgCanSetDlg::loadDbc(QString dbcFile)
{
    if(dbcFile.isEmpty())
        return;
...
void ZlgCanSetDlg::loadMsgToModel()
{
...
QStringList ZlgCanSetDlg::getMsgNameList(QTreeView *treeView)
{
    QStringList msgList;
...
void ZlgCanSetDlg::loadFramesToReadTreeView(QList<BO_ *> boList)
{
...
void ZlgCanSetDlg::loadFramesToWriteTreeView(QList<BO_ *> boList)
{
...
BO_ *ZlgCanSetDlg::getBoById(quint32 id)
{
    if(m_cluster == NULL)
        return NULL;
...
BO_ *ZlgCanSetDlg::getBoByName(QString boName)
{
    if(m_cluster == NULL)
        return NULL;
    if(m_cluster->m_boList.isEmpty())
...
QList<BO_ *> ZlgCanSetDlg::getBoListByNames(QStringList boNameList)
{
    QList<BO_*> boList;
...
SG_ *ZlgCanSetDlg::getSgByName(BO_ *bo, QString sgName)
{
    if(!bo)
        return NULL;
...
QString ZlgCanSetDlg::getCanSetFilePath() const
{
    return canSetFilePath;
}

void ZlgCanSetDlg::accept()
{
    //writeSettingXml();
    saveXml();
    QDialog::accept();
...
QString ZlgCanSetDlg::getCanSetFileName() const
{
    return canSetFileName;
}

void ZlgCanSetDlg::on_pb_loadDatabase_clicked()
{
...
void ZlgCanSetDlg::on_pb_reset_clicked()
{
