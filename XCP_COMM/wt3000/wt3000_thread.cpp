...
bool WT3000_Thread::connectDevice()
{
    ...
    if(sta!=0)
    {
        qInfo()<<Q_FUNC_INFO<<"WT3000 connection error."<<sta;
        return false;
    }
    ...
    {
        TmcFinish(deviceID);
        qInfo()<<Q_FUNC_INFO<<"WT3000 set term error.";
        return false;
    }
    ...
    {
        TmcFinish(deviceID);
        qInfo()<<Q_FUNC_INFO<<"WT3000 set time out error.";
        return false;
    }
   ...
   {
       TmcFinish(deviceID);
       qInfo()<<Q_FUNC_INFO<<"WT3000 set remote mode error.";
       return false;
   }
...
bool WT3000_Thread::disconnectDevice()
{
    qInfo()<<"WT3000 设备断开连接";
    emit wtFunStateChanged(0, funIndex);
    wtState = 0;
   ...
   if(sta!=0)
   {
    ...
    if(sta!=0)
    {
        qInfo()<<Q_FUNC_INFO<<"WT3000 disconnect error.";
        return false;
    }
    qInfo()<<"WT3000 设备断开连接成功";

    return true;
}

void WT3000_Thread::setIPAddress(QString address)
{
    IPAddress=address;
}

void WT3000_Thread::setFunInfo(FunStruct *value, int index)
{
    fun = value;
    funIndex = index;
}

void WT3000_Thread::init()
{
    readSetting();
...
void WT3000_Thread::createSM()
{
    smKey = "RP_WT3000";

    if(!sm_wt3000)
    {
        sm_wt3000 = new QSharedMemory(smKey);
    }
    sm_wt3000->create(200);

}

void WT3000_Thread::deleteSM()
{
    if(sm_wt3000)
    {
        sm_wt3000->detach();
...
void WT3000_Thread::writeSetting()
{

}

void WT3000_Thread::readSetting()
{
    ...
    qDebug()<<"In wt3000 fun thread: read setting: "<<setFilePath;
    QSettings setting(setFilePath, QSettings::IniFormat);

    IPAddress = setting.value("IP_Address").toString();
}

void WT3000_Thread::createPams()
{
...
QString WT3000_Thread::getSmKey() const
{
    return smKey;
}

QList<PARAM *> WT3000_Thread::getReadPamList() const
{
    return readPamList;
}

int WT3000_Thread::getWtState() const
{
    return wtState;
}

void WT3000_Thread::setStopFlg(bool value)
{
    stopFlg = value;
}

void WT3000_Thread::run()
{
  ...
  //    }
    ...
    while(!stopFlg)
    {
        ...
        //copyToSharedMemory(sm_wt3000, 0, (char*)&currentTime, 8);
        copyToSharedMemory(sm_wt3000, 0, (char*)&time_100ns, 8);
...
//            qInfo()<<Q_FUNC_INFO<<"Receive error,disconnect from WT3000.";
        ...
        else
        {
            ...
            //所有float数据更新至共享内存中。
            copyToSharedMemory(sm_wt3000, 8, buff + 6, 4 * 48);
        ...
        if(sta!=0)
        {
//            qInfo()<<Q_FUNC_INFO<<"Send wait require error,disconnect from WT3000.";
            //stopFlg=true;
            //emit reconnect();
        }
    }
...
void WT3000_Thread::wtDataUpdatedSlot()
{
    if(!sm_wt3000) return;
...
void WT3000_Thread::reverseData_4bytes(char *src, char *dst)
{
    if(!src || !dst) return;
