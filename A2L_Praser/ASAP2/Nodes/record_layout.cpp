...
TokenTyp RECORD_LAYOUT::parseOptPar()
{
    ...
    nameOptPar.insert("ALIGNMENT_INT64", ZeroOrOne);
    nameOptPar.insert("STATIC_RECORD_LAYOUT", ZeroOrOne);
    nameOptPar.insert("RESERVED", ZeroOrMore);
    ...
    else
    {
        ...
        while (token == BlockBegin || token == Keyword || token == PrgType)
        {
            //Nodes
            if (token == BlockBegin)
            {
            ...
            else if (lex->getLexem() == "RESERVED")
            {
                std::string lexem = lex->getLexem();
                //qDebug()<<"find reserved in record_layout"<<QString::fromStdString(lexem);
                if (nameOptPar.contains(lexem))
                {
                    if (nameOptPar.value(lexem) == ZeroOrOne)
                    {
