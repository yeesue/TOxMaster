...
Factory<Node,PAGE> PAGE::nodeFactory;

// Functions (Predicate)
bool nodeLessThan( const Node *a, const Node *b );
bool itemLessThan( const Item *a, const Item *b );

PAGE::PAGE( Node *parentNode)
    : Node(parentNode, parentNode->lex, parentNode->errorList)
{
    //get grammar
    A2lGrammar* gram = parentNode->lex->grammar;
    namePar = &gram->page.namePar;
    typePar = &gram->page.typePar;
    factoryOptNode = &gram->page.factoryOptNode;
    factoryOptItem = &gram->page.factoryOptItem;
    ...
    */
    ...
    occOptPar->insert("INIT_SEGMENT", ZeroOrOne);
    ...
    {
        token = nextToken();
        if (token == Keyword && lex->getLexem() == "PAGE")
        {
            //Sort the childNodes
        ...
        else
        {
            ...
            this->showError("expected token : BlockEnd PAGE\nfind token : " + s );
        }
    }
    else
    {
        ...
        this->showError("expected end PAGE\nfind : " + s1 + " " + s2 );
    }
...
void PAGE::parseFixPar(QList<TokenTyp> *typePar)
{
...
TokenTyp PAGE::parseOptPar(QHash<QString, Occurence> *nameOptPar)
{
...
QMap<std::string, std::string> *PAGE::getParameters()
{
...
char* PAGE::getPar(std::string str)
{
