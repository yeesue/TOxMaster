...
void MEASUREMENT::parseFixPar(QList<TokenTyp> *typePar)
{
    ...
    for (int i = 0; i < typePar->count(); i++)
    {
        ...
        else if(typePar->at(i) == Float && token == Hex)
        {
            char *c = new char[parentNode->lex->getLexem().length()+1];
            strcpy(c, parentNode->lex->getLexem().c_str());
            parameters.append(c);
        }
        else if(typePar->at(i) == Float && token == Identifier)
        {
            //char *c = new char[parentNode->lex->getLexem().length()+1];
            //strcpy(c, parentNode->lex->getLexem().c_str());
            //parameters.append(c);
            //发现实例：lower和upper limit的值显示lower和upper
            char *c = new char[1];
            strcpy(c, "0");
            parameters.append(c);
        }
...
TokenTyp MEASUREMENT::parseOptPar()
{
    ...
    nameOptPar.insert("LAYOUT", ZeroOrOne);
    nameOptPar.insert("MATRIX_DIM", ZeroOrOne);
    nameOptPar.insert("READ_WRITE", ZeroOrOne);
    nameOptPar.insert("SYMBOL_LINK", ZeroOrOne);
    nameOptPar.insert("PHYS_UNIT", ZeroOrOne);
