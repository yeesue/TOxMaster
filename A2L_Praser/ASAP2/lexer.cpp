...
//            token = begin(in, ch);
    ...
    //qDebug()<<"pos = "<<in->pos()<<"; position = "<<position;
    return token;
}
...
TokenTyp A2lLexer::begin(char ch)
{
    ...
    //qDebug()<<"lexem = "<<QString::fromStdString(lexem);
...
TokenTyp A2lLexer::string()
{
    ...
    while (!exit)
    {
        //&& buffer->getValue() != '\n' && buffer->getValue() != 0//前面这段从while循环条件中删除，避免乱码以及跨行的字符串
        while (buffer->getValue() != '"' && buffer->getValue() != '\\' )
        {
            lexem += buffer->getAndClear();
            buffer->read(in);
        }
...
TokenTyp A2lLexer::getPartialString()
{
    ...
    while (isDigit(_check) || isLetter(_check) || _check == '_' || _check == '[' || _check == ']')
    {
        lexem += buffer->getAndClear();
        buffer->read(in);
