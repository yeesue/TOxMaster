...
// Static helper for string interning
static char* internNodeName(const char* name)
{
    if (!name) return nullptr;
    
    // Intern the string to reduce memory for repeated keywords
    QString interned = a2l::NodePoolManager::instance().internString(name);
    
    // Return persistent C-string
    // Note: This creates a copy, but the QString is shared
    static QHash<QString, QByteArray> internedCache;
    static QMutex cacheMutex;
    
    QMutexLocker locker(&cacheMutex);
    if (!internedCache.contains(interned)) {
        internedCache[interned] = interned.toLocal8Bit();
    }
    return internedCache[interned].data();
}
...
Node *Node::child(QString str, bool bin)
{
   if (bin)
   {
        ...
        {
            //qDebug()<<"node i name = "<<QString(((Node*)*i)->name);
            //qDebug()<<"node name = "<<QString(node.name);
