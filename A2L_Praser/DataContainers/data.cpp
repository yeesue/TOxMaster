...
Data::Data(CHARACTERISTIC *node, PROJECT *pro, HexFile *hexFile, bool modif) : Node(node->name), QObject()
{
            ...
            if(record_layout)
            {
                ...
                foreach (Item *item, record_layout->optItems)
                {
                    ...
                    else if (type == "NO_AXIS_PTS_X")
                    {
                        ...
                        QString val = hexParent->getHexValue(axisPtsX->getPar("Address"), offset, nbyte, byteOrderX);
                        bool bl;
                    ...
                    else if (type == "AXIS_PTS_X")
                    {
                        ...
                        addressX = QString(axisPtsX->getPar("Address")).toUInt(&bl, 16) + offset;
    ...
    if (axisDescrY)
    {
        ...
        if (typeAxisY.compare("COM_AXIS") == 0)
        {
            ...
            if(record_layout)
            {
                //read each element of Y_RECORD_LAYOUT
                bool bl;
                int offset = 0;
                foreach (Item *item, record_layout->optItems)
                {
                    ...
                    else if (type == "NO_AXIS_PTS_X")
                    {
                        std::string datatype = ((NO_AXIS_PTS_X*)item)->getPar("Datatype");
                        int nbyte = hexParent->getNumByte(datatype);
                        QString val = hexParent->getHexValue(axisPtsY->getPar("Address"), offset, nbyte, byteOrderY);
                        bool bl;
                    ...
                    else if (type == "AXIS_PTS_X")
                    {
                        ...
                        addressY = QString(axisPtsY->getPar("Address")).toUInt(&bl, 16) + offset;
    ...
    if(compu_methodZ)
    {
        //qDebug()<<"current compuMethod:"<<compu_methodZ->name;
    }
    ...
    if(record_layout)
    {
        ...
        foreach (Item *item, record_layout->optItems)
        {
            ...
            else if (type.compare("NO_AXIS_PTS_X") == 0)
            {
                std::string datatype = ((NO_AXIS_PTS_X*)item)->getPar("Datatype");
                int nbyte = hexParent->getNumByte(datatype);
                double address = QString(node->getPar("Address")).toUInt(&bl, 16) + offset;
            ...
            else if (type.compare("AXIS_PTS_X") == 0)
            {
                ...
                datatypeX = ((AXIS_PTS_X*)item)->getPar("Datatype");
                int Xnbyte = hexParent->getNumByte(datatypeX);
                addressX = QString(node->getPar("Address")).toUInt(&bl, 16) + offset;
            ...
            else if (type.compare("AXIS_PTS_Y") == 0)
            {
                ...
                int Ynbyte = hexParent->getNumByte(datatypeY);
                addressY = QString(node->getPar("Address")).toUInt(&bl, 16) + offset;
    ...
    {
        qDebug()<<"charac:"<<node->name<<",record layout not found:"<<deposit;
    }
        ...
        if (typeAxisX.compare("COM_AXIS") == 0)
        {
            ...
            foreach (Item *item, record_layout->optItems)
            {
                ...
                else if (type == "NO_AXIS_PTS_X")
                {
                    ...
                    QString val = srecParent->getHexValue(axisPtsX->getPar("Address"), offset, nbyte, byteOrderX);
                    bool bl;
                    nPtsAxisX = val.toInt(&bl,16);
                    offset += nbyte;
        ...
        if (typeAxisY.compare("COM_AXIS") == 0)
        {
            ...
            foreach (Item *item, record_layout->optItems)
            {
                ...
                else if (type == "NO_AXIS_PTS_X")
                {
                    std::string datatype = ((NO_AXIS_PTS_X*)item)->getPar("Datatype");
                    int nbyte = srecParent->getNumByte(datatype);
                    QString val = srecParent->getHexValue(axisPtsY->getPar("Address"), offset, nbyte, byteOrderY);
                    bool bl;
                    nPtsAxisY = val.toInt(&bl,16);
                    offset += nbyte;
...
Data::Data(AXIS_PTS *node, PROJECT *pro, HexFile *hexFile, bool modif) : Node(node->name), QObject()
{
    ...
    foreach (Item *item, record_layout->optItems)
    {
        ...
        else if (type == "NO_AXIS_PTS_X")
        {
            std::string datatype = ((NO_AXIS_PTS_X*)item)->getPar("Datatype");
            int nbyte = hexParent->getNumByte(datatype);
            QString val = hexParent->getHexValue(node->getPar("Address"), offset, nbyte, byteOrderZ);
            bool bl;
...
Data::Data(AXIS_PTS *node, PROJECT *pro, SrecFile *srecFile, bool modif) : Node(node->name), QObject()
{
    ...
    foreach (Item *item, record_layout->optItems)
    {
        ...
        else if (type == "NO_AXIS_PTS_X")
        {
            std::string datatype = ((NO_AXIS_PTS_X*)item)->getPar("Datatype");
            int nbyte = srecParent->getNumByte(datatype);
            QString val = srecParent->getHexValue(node->getPar("Address"), offset, nbyte, byteOrderZ);
            bool bl;
            nPts = val.toInt(&bl,16);
            offset += nbyte;
        ...
        if (compu_methodZ)
        {
            ...
            else if (convType.toLower() == "tab_verb")
            {
                ...
                for (int i = 0; i < decValues.count(); i++)
                {
                    ...
                    //list.append(compuVTabAxisZ->getValue((int)dec));   //get value from COMPU_VTAB, may cause Error!!
                    list.append(QString::number((int)dec));
                }
