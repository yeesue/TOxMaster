...
#include <sstream>
#include "core/memory/node_pool.h"
...
void A2l::parse()
{
    ...
    // Initialize memory pool for parsing session
    // Estimate node count based on file size (rough estimate: 1 node per 500 bytes)
    size_t estimatedNodes = QFileInfo(fullA2lName).size() / 500;
    a2l::NodePoolManager::instance().initialize(
        qMax(estimatedNodes, (size_t)1000),
        estimatedNodes * 5
    );
    ...
    auto stats = a2l::NodePoolManager::instance().getStats();
    QString memInfo;
    memInfo.sprintf("Memory pool: %zu strings interned, ~%zu KB saved",
        stats.stringCount, stats.totalMemoryEstimate / 1024);
    this->outputList.append(memInfo);
...
bool A2l::trunkA2l(QString &str, QString &str1, QString &str2)
{
    ...
    // Optimized: use QString::indexOf instead of QRegExp for better performance
    int a = str.indexOf("/begin MODULE");
    int b = str.lastIndexOf("/end MODULE");
    ...
    if ((a + 13 < middle) && (middle < b))
    {
        ...
        // Optimized: use QString operations instead of QRegExp
        QString nodeType = "";
        int num = str1.length();
        int lastBeginChar = 0;
        static const QStringList targetTypes = {
            "CHARACTERISTIC", "MEASUREMENT", "RECORD_LAYOUT", 
            "FUNCTION", "AXIS_PTS"
        };

        while (!targetTypes.contains(nodeType))
        {
            // Find "/begin " pattern
            ...
            // Extract the node type after "/begin "
            int typeStart = beginIdx + 7; // length of "/begin "
            int typeEnd = str1.indexOf(' ', typeStart);
            if (typeEnd < 0) {
                typeEnd = str1.indexOf('\n', typeStart);
            }
            if (typeEnd < 0) {
                typeEnd = str1.indexOf('\r', typeStart);
            }
...
void A2l::readSubset()
{
    if (a2lFile)
    {
        ...
        if (project)
        {
            ...
            //qDebug()<<"module num"<<listModule.count();
