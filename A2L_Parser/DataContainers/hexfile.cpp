    ...
    : QObject(parent) , DataContainer(parentWP, module)
{
    ...
    //MOD_COMMON values (BYTE_ORDER, ALIGNEMENT_ORDER,...)
    ...
    if (modCommon)
    {
        ...
        if (item4)
        {
            ...
            commonAlignmentByte.insert("FLOAT32_IEEE", str.toInt(&bl,10));
        }
        else
        {
...
{
    MOD_PAR *modePar = (MOD_PAR*)a2lProject->getNode("MODULE/" + getModuleName() + "/MOD_PAR");
    qDebug()<<"ModePar:"<<modePar->getPar("Comment");
    ...
    if (modePar)
    {
        ...
        if (epk)
        {
            ...
            if (addr_epk)
            {
                ...
                qDebug()<<"compare EPK from A2L and HexFile.";
...
Data* HexFile::runCreateDataMapped(const QString &str)
{
    ...
    if (nodeChar)
    {
        ...
        if (label)
        {
            ...
            QString add = charac->getPar("Address");
    ...
    if (nodeAxis && !found)
    {
        ...
        if (label2)
        {
            ...
            QString add = axis->getPar("Address");
...
Data* HexFile::runCreateDataMapped2(const Node *node)
{
    ...
    {
        //qDebug()<<"Current Charac:"<<node->name;
    ...
    {
        //qDebug()<<"Current AxiPts:"<<node->name;
    ...
    if (type.endsWith("CHARACTERISTIC"))
    {
        ...
        else if(type.toLower() == "curve")
        {
            ...
            else
            {
                ...
                bool bl;
                double addr = QString(node->getPar("Address")).toUInt(&bl, 16);
        ...
        else if(type.toLower() == "map")
        {
            ...
            else
            {
                ...
                bl;
                addr = QString(node->getPar("Address")).toUInt(&bl, 16);
...
void HexFile::checkDisplay()
{
    ...
    if (!parentNode->getParentNode()->isChild(this->parentNode))
    {
        ...
        {
            MainWindow *mdi = getParentWp()->parentWidget;
