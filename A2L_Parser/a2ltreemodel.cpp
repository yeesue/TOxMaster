...
Node *A2lTreeModel::findNodeByName(QString name)
{
    Node *node = rootNode;
    if (node->name == name)
        return node;
    if (node->childCount() > 0)
        return findChildNodeByName(node, name);

}

Node *A2lTreeModel::findChildNodeByName(Node *node, QString name)
{
    if (node == NULL)
        return NULL;
    if (node->childCount() == 0)
    ...
    {
        Node *childNode = node->childNodes.at(i);
        //qDebug()<<"CurrentNode Name:"<<childNode->name;
        if (childNode->name == name)
            return childNode;

        if (childNode->childCount() > 0)
        {
            Node *grandChildNode = findChildNodeByName(childNode, name);
